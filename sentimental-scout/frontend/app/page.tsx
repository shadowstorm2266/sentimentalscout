...
export default function Home() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (res.ok) {
        WebApp.close(); // User pays in Telegram chat
      } else {
          ...
          {result && (
            ...
            >
              ...
              <div className="space-y-0">
                <SignalRow
                  label="Social Volume"
                  value={result.signals.social_volume.toLocaleString()}
                  bar={Math.min(100, result.signals.social_volume / 500)}
                />
                <SignalRow
                ...
                <SignalRow
                  label="Dev Activity"
                  value={`${result.signals.dev_activity}/100`}
                  bar={result.signals.dev_activity}
                />
                <SignalRow
                  label="Fear & Greed"
                  value={`${result.signals.fear_greed_index}/100`}
                  bar={result.signals.fear_greed_index}
                />
                <SignalRow
                  label="Price Momentum"
                  value={`${result.signals.price_momentum > 0 ? "+" : ""}${result.signals.price_momentum}%`}
                />
              </div>
