...
function StatBox({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
    <div className="rounded-lg border border-white/[0.07] bg-white/[0.03] px-3 py-2.5 flex flex-col gap-0.5">
      <span className="text-[9px] text-[#555] font-mono uppercase tracking-[0.15em]">{label}</span>
      <span className="text-[13px] font-bold font-mono leading-tight" style={{ color: accent ?? "#e0e0e0" }}>{value}</span>
...
function BarRow({ label, value, bar, accent }: { label: string; value: string; bar?: number; accent?: string }) {
  ...
  return (
    <div className="flex items-center justify-between py-[7px] border-b border-white/[0.04] last:border-0">
      <span className="text-[10px] text-[#555] font-mono uppercase tracking-widest w-24 shrink-0">{label}</span>
      <div className="flex items-center gap-2 flex-1 justify-end">
        ...
        <span className="text-[11px] font-mono" style={{ color: accent ?? "#bbb" }}>{value}</span>
      </div>
    </div>
  );
}
...
function ScorePill({ label, score, weight }: { label: string; score: number; weight: number }) {
  ...
  return (
    <div className="rounded-lg border border-white/[0.07] bg-white/[0.03] p-2.5 flex flex-col items-center gap-1.5">
      <span className="text-[8px] text-[#555] font-mono uppercase tracking-widest">{label}</span>
      <span className="text-[15px] font-bold font-mono" style={{ color: c, textShadow:`0 0 10px ${c}60` }}>
        {score > 0 ? "+" : ""}{score.toFixed(0)}
      </span>
      <div className="w-full h-[2px] bg-white/10 rounded-full overflow-hidden">
        <div className="h-full rounded-full" style={{ width:`${pct}%`, background:c, transition:"width 0.8s ease" }}/>
      </div>
      <span className="text-[8px] text-[#444] font-mono">W·{(weight*100).toFixed(0)}%</span>
    </div>
  );
}
        ...
        <div className="relative z-10 px-4 space-y-3">
          ...
          {result && !loading && (
            <div className="rounded-xl border p-4 space-y-4"
              ...
              {/* Signal bars */}
              <div className="rounded-lg border border-white/[0.07] bg-white/[0.02] px-4 py-1">
              ...
              {/* Score breakdown */}
              <div>
                <div className="text-[9px] text-[#444] font-mono uppercase tracking-widest mb-2">Score Breakdown</div>
