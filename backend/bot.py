...
SOCIAL_API_KEY: str   = os.environ.get("SOCIAL_SEARCHER_KEY", "")

TG_API         = f"https://api.telegram.org/bot{BOT_TOKEN}"
COINGECKO_BASE = "https://api.coingecko.com/api/v3"
HELIUS_BASE    = "https://mainnet.helius-rpc.com"
SOCIAL_BASE    = "https://api.social-searcher.com/v2"
...
async def fetch_social_signal(session: aiohttp.ClientSession, ticker: str) -> dict:
    """S signal — mention Z-score + sentiment ratio."""
    current_mentions = 0
    avg_mentions     = 100
    sentiment_ratio  = 0.0
    ...
    if SOCIAL_API_KEY:
        try:
            params = {"key": SOCIAL_API_KEY, "q": f"${ticker}",
                      "network": "twitter,reddit", "limit": 100}
            async with session.get(f"{SOCIAL_BASE}/search", params=params,
                                   timeout=aiohttp.ClientTimeout(total=8)) as r:
                if r.status == 200:
                    data     = await r.json()
                    posts    = data.get("posts", [])
                    current_mentions = len(posts)
                    pos = sum(1 for p in posts if (p.get("sentiment") or "").lower() == "positive")
                    neg = sum(1 for p in posts if (p.get("sentiment") or "").lower() == "negative")
                    sentiment_ratio = (pos - neg) / max(current_mentions, 1)
                    avg_mentions = (data.get("meta", {}).get("statistic", {})
                                    .get("avg_mentions", 100) or 100)
        except Exception as exc:
            log.warning("Social error [%s]: %s", ticker, exc)

    if current_mentions == 0:
        rng              = random.Random(ticker.upper() + "soc" + str(int(time.time() // 3600)))
        current_mentions = rng.randint(50, 50_000)
        avg_mentions     = rng.randint(500, 20_000)
        sentiment_ratio  = rng.uniform(-0.4, 0.6)

    stddev   = max(avg_mentions * 0.5, 1)
    z_score  = (current_mentions - avg_mentions) / stddev
    vol_comp = clamp(z_score / 3 * 60, -60, 60)
    sen_comp = clamp(sentiment_ratio * 40, -40, 40)
    S        = clamp(vol_comp + sen_comp)

    # Bot noise dampening
    if z_score > 4 and abs(sentiment_ratio) < 0.1:
        S *= 0.5
    ...
    return {
        ...
        "source": "social_searcher" if SOCIAL_API_KEY else "mock",
    }
...
def apply_rug_sense(score: float, onchain: dict, market: dict) -> tuple[float, bool]:
    rug = False
...
async def run_scoring_engine(ticker: str) -> dict:
    ...
    final_score, rug_hit = apply_rug_sense(raw_score, onchain, market)
