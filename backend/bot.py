...
BOT_USERNAME: str     = os.environ.get("BOT_USERNAME", "OnChainOracleBot")
...
BONUS_SCANS_PER_REFERRAL = 5
REFERRAL_SCANS_REWARD    = 5  # scans given to referrer per successful referral

def generate_referral_code(telegram_id: int) -> str:
    import hashlib
    h = hashlib.md5(str(telegram_id).encode()).hexdigest()[:6].upper()
    return f"OC{h}"

def upsert_user(tg_user: dict, referred_by_code: str | None = None) -> dict:
    code = generate_referral_code(tg_user["id"])
    data = {
    ...
    user = (res.data[0] if res.data else None) or get_user(tg_user["id"]) or data

    # Process referral if this is a new user joining via referral link
    if referred_by_code and not user.get("referred_by"):
        try:
            ...
            if ref_res.data and ref_res.data[0]["telegram_id"] != tg_user["id"]:
                ...
                # Check not already referred (race condition guard)
                existing = (
                ...
                if not existing.data:
                    ...
                    supabase.table("referrals").insert({
                        ...
                        "referred_id":     user["id"],
                        "telegram_id_ref": tg_user["id"],
                        "bonus_awarded":   REFERRAL_SCANS_REWARD,
                    }).execute()

                    # Award bonus scans to referrer
                    supabase.table("users").update({
                        "bonus_scans": (referrer.get("bonus_scans") or 0) + REFERRAL_SCANS_REWARD
                    }).eq("id", referrer["id"]).execute()

                    log.info("Referral credited: %s → %s (+%d scans)",
                             referrer["telegram_id"], tg_user["id"], REFERRAL_SCANS_REWARD)

                    # Notify referrer
                    asyncio.create_task(tg_post("sendMessage", {
                        "chat_id": referrer["telegram_id"],
                        "text": (
                            f"🔮 *+{REFERRAL_SCANS_REWARD} bonus scans\\!*\n\n"
                            f"Someone just joined OnChain Oracle using your referral link\\.\n"
                            f"You now have *{(referrer.get('bonus_scans') or 0) + REFERRAL_SCANS_REWARD} bonus scans* banked\\.\n\n"
                            f"Keep sharing to earn more 🚀"
                        ),
                        "parse_mode": "MarkdownV2",
                    }))
        except Exception as exc:
            log.warning("Referral processing failed: %s", exc)
...
def can_scan(user: dict) -> bool:
    ...
    try:
        ...
        pass
    total_allowed = FREE_SCANS_PER_DAY + (user.get("bonus_scans") or 0)
    return user["scans_today"] < total_allowed

def scans_remaining(user: dict) -> int | str:
    if user["subscription"] == "pro":
        return "∞"
    total_allowed = FREE_SCANS_PER_DAY + (user.get("bonus_scans") or 0)
    return max(0, total_allowed - user["scans_today"])
...
async def fetch_market_signal(session: aiohttp.ClientSession, ticker: str) -> dict:
    ...
    Uses /coins/markets (lighter, less rate-limited than /coins/{id}).
    """
            ...
            async with session.get(url, params=params, headers=headers,
                ...
                if r.status == 200:
                    ...
                    if data and len(data) > 0:
                        ...
                        vol_mcap     = (volume_24h / max(market_cap, 1)) * 100
                    ...
                    log.warning("CoinGecko status %s for %s", r.status, ticker)
        except Exception as exc:
...
async def run_scoring_engine(ticker: str) -> dict:
    """
    Orchestrates all three signal fetches concurrently.
    Each signal has its own fallback — a single API failure
    ...
    # Graceful per-signal fallback
    rng = random.Random(ticker.upper() + str(int(time.time() // 3600)))

    def safe_market(r: any) -> dict:
        if isinstance(r, Exception):
            log.warning("Market signal exception [%s]: %s", ticker, r)
            pc = rng.uniform(-8, 12)
            vm = rng.uniform(2, 15)
            return {"score": round(clamp(pc*5*0.7+(vm-5)*4*0.3),2),
                    "price_change_24h": round(pc,2), "volume_24h_usd": 0,
                    "market_cap_usd": 0, "vol_mcap_ratio": round(vm,2), "source":"fallback"}
        return r

    def safe_onchain(r: any) -> dict:
        if isinstance(r, Exception):
            log.warning("Onchain signal exception [%s]: %s", ticker, r)
            return {"score": 0.0, "max_holder_pct": 0.0, "rug_flag": False,
                    "large_tx_count": 0, "whale_bias": "Unknown ❓", "source":"fallback"}
        return r

    def safe_social(r: any) -> dict:
        if isinstance(r, Exception):
            log.warning("Social signal exception [%s]: %s", ticker, r)
            return {"score": 0.0, "current_mentions": 0, "avg_mentions": 0,
                    "z_score": 0.0, "sentiment_ratio": 0.0,
                    "buzz_level": "Unknown ❓", "source":"fallback"}
        return r

    market  = safe_market(results[0])
    onchain = safe_onchain(results[1])
    social  = safe_social(results[2])
    ...
    is_memecoin = (not is_major) and (market_cap > 0) and (market_cap < 20_000_000)
    ...
    # Flag which signals used fallback data
    data_sources = {
        "market":  market.get("source", "unknown"),
        "onchain": onchain.get("source", "unknown"),
        "social":  social.get("source", "unknown"),
    }
    partial = any(v == "fallback" for v in data_sources.values())
...
async def get_sentiment(req: SentimentRequest):
    ...
    # Basic validation
    if not ticker:
        raise HTTPException(400, "Please enter a ticker symbol.")
    if len(ticker) > 10:
        raise HTTPException(400, "Ticker too long. Max 10 characters.")
    if not ticker.isalnum():
        raise HTTPException(400, "Ticker must be letters and numbers only.")

    user = get_user(req.telegram_id)
    if not user:
        raise HTTPException(404, "User not found. Close and reopen the app.")
    if not can_scan(user):
        raise HTTPException(429,
            f"You've used all your scans today. "
            f"Share your referral link for +{REFERRAL_SCANS_REWARD} free scans, "
            f"or upgrade to Pro for unlimited ⭐")

    try:
        result = await run_scoring_engine(ticker)
    except Exception as exc:
        log.exception("Scoring engine failed [%s]: %s", ticker, exc)
        raise HTTPException(500, "Scan failed. Please try again in a moment.")

    # Warn user if data is partial
    if result.get("partial_data"):
        result["warning"] = "Some data sources were unavailable. Results may be less accurate."
...
async def create_invoice(req: PayInvoiceRequest):
    payload = {
        "chat_id":        req.chat_id,
        "title":          "OnChain Oracle Pro",
        "description":    "Unlimited scans · Rug-Sense alerts · Whale tracking · AI degen verdicts · No daily limits",
        "payload":        json.dumps({"telegram_id": req.telegram_id, "plan": "pro"}),
        "provider_token": "",
        "currency":       "XTR",
        "prices":         [{"label": "Oracle Pro (30 days)", "amount": PRO_STARS_PRICE}],
    }
...
async def get_user_profile(telegram_id: int):
    ...
    referral_link = f"https://t.me/{os.environ.get('BOT_USERNAME', 'OnChainOracleBot')}?start={user.get('referral_code','')}"
        ...
        if msg := update.get("message"):
            ...
            if payment := msg.get("successful_payment"):
                ...
                await tg_post("sendMessage", {
                    "chat_id":    msg["chat"]["id"],
                    "text": (
                        "⭐ *Payment confirmed\\!* You're now an Oracle Pro member\\.\n\n"
                        "Unlimited scans · Rug\\-Sense · Whale tracking · AI degen verdicts 🔮\n\n"
                        "The oracle is yours\\. Go find some alpha\\."
                    ),
                    "parse_mode": "MarkdownV2",
                })
                return
            ...
            if text.startswith("/start"):
                # Extract referral code from deep link e.g. /start OC1A2B3C
                parts = text.strip().split()
                ref_code = parts[1] if len(parts) > 1 else None
                upsert_user(tg_user, referred_by_code=ref_code)
                await tg_post("sendMessage", {
                    "chat_id":    msg["chat"]["id"],
                    "text": (
                        "🔮 Welcome to *OnChain Oracle*!\n\n"
                        "_The oracle degens trust before aping in._\n\n"
                        "Scan any crypto in seconds — get whale posture, "
                        "rug risk, social buzz, and an AI verdict telling "
                        "you exactly what the signals mean.\n\n"
                        "🔍 Live market data · 🐋 Whale tracking\n"
                        "☠️ Rug\\-Sense · 🔮 AI degen verdicts\n\n"
                        "No noise\\. Just alpha\\.\n\n"
                        "Free: 3 scans/day · Pro ⭐: unlimited"
                    ),
                    "parse_mode": "MarkdownV2",
                    "reply_markup": {
                        "inline_keyboard": [[{
                            "text":    "🔮 Open OnChain Oracle",
